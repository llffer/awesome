...
local function worker(args)
	...
	local yes_internet = wibox.widget({
		{
			...
			image = ICON_DIR .. "wireless-full.png",
			resize = false,
		},
	...
	local no_internet = wibox.widget({
		{
			widget = wibox.widget.imagebox,
			image = ICON_DIR .. "wireless-disconnected.png",
	...
	local function net_update()
		connected = false
		awful.spawn.easy_async('bash -c "nc -z 8.8.8.8 53 >/dev/null 2>&1"', function(_, _, _, exit_code)
			if exit_code == 0 then
				connected = true
			end
			if connected then
